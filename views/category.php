...
<section style="background-color:#d8e0d7">
	...
	<div class="home-header ml-4 mt-2">
		<?php 
			...
			$cat_nameSQl = "SELECT * FROM category WHERE $cat_id = cat_id";
			$run_cat_nameSQl = mysqli_query($conn, $cat_nameSQl);
			$name = mysqli_fetch_array($run_cat_nameSQl);

		 ?>
		<h3 class="text-left"><?php echo $name['cat_name']; ?></h3>
	</div>
</section>
<div class="news">
...
if (isset($_GET['search_nav'])) {
 	...
 	$post_query = "SELECT * FROM news WHERE title LIKE '%$search_key%' ";
 }
 
 else{
 	$post_query = "SELECT * FROM news WHERE $cat_id = cat_id";
